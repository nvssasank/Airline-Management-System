...
const CrewMemberManagement = () => {
  ...
  useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      const results = crewMembers.filter(crew => 
        ...
        crew.airline.airline_code.toLowerCase().includes(searchTerm.toLowerCase())
      );
  ...
  const handleEditClick = (crew) => {
    ...
    setFormData({
      airline: crew.airline.airline_id,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Crew Member Management</h1>
      ...
      <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
        <div className="relative flex items-center w-full md:w-2/3">
          ...
          <input
            type="text"
            placeholder="Search by name, staff ID, position, or airline code..."
            ...
            {currentCrewMembers.length > 0 ? (
              currentCrewMembers.map((crew) => (
                <tr key={crew.crew_id} className="hover:bg-gray-50">
                  <td className="py-3 px-4 font-medium">{crew.first_name} {crew.last_name}</td>
                  <td className="py-3 px-4">{crew.staff_id}</td>
                  <td className="py-3 px-4">{crew.position}</td>
                  <td className="py-3 px-4">{crew.airline.airline_code}</td>
          ...
          <div className="bg-white rounded-lg w-full max-w-2xl p-6 shadow-xl">
            ...
            <form onSubmit={handleSubmitAdd}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-gray-700 mb-1">Certification (optional)</label>
                  <input
                    type="text"
                    name="certification"
                ...
                <div>
                  <label className="block text-gray-700 mb-1">Duty Hours (optional)</label>
                  <input
                    type="number"
                    name="duty_hours"
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-md p-6 shadow-xl">
            ...
            <p className="mb-6">
              Are you sure you want to delete the crew member <span className="font-semibold">{currentCrewMember?.first_name} {currentCrewMember?.last_name}</span> (Staff ID: <span className="font-semibold">{currentCrewMember?.staff_id}</span>)? This action cannot be undone.
            </p>
